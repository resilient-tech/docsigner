...
Gated (DOCSIGNER_E2E_BROWSER=1, needs Chrome + `pip install playwright` and
...
def test_bridge_event_names_match_contract():
    ...
    assert "org.docsigner.request" in content
    assert "org.docsigner.response" in content
...
@pytest.mark.skipif(
    os.environ.get("DOCSIGNER_E2E_BROWSER") != "1",
    reason="browser path: set DOCSIGNER_E2E_BROWSER=1 (needs Chrome + playwright)",
)
