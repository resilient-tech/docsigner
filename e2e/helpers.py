...
# The blank PDF is the same one core's tests use; core/tests is on the path via
# pytest.ini. Re-exported so this module stays the single import for e2e tests.
from helpers_core import make_blank_pdf  # noqa: F401
...
def make_signer_cert(key_type: str = "rsa", cn: str | None = None):
    ...
    name = x509.Name(
        [
            x509.NameAttribute(NameOID.COUNTRY_NAME, os.environ.get("E2E_SIGNER_COUNTRY", "IN")),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, os.environ.get("E2E_SIGNER_ORG", "DocSigner Tests")),
            x509.NameAttribute(NameOID.COMMON_NAME, cn or os.environ.get("E2E_SIGNER_CN", "Test Signer")),
        ]
    )
    ...
    email = os.environ.get("E2E_SIGNER_EMAIL", "")
    if email:
