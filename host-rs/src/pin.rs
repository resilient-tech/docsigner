...
pub fn get_pin(token_label: &str) -> Result<String> {
    if let Ok(pin) = std::env::var(ENV_VAR) {
        if !pin.is_empty() {
            return Ok(pin);
        }
    }
    match prompt(token_label) {
...
#[cfg(target_os = "macos")]
fn prompt(token_label: &str) -> Option<String> {
    ...
    let script = format!(
        "try\n  \
           text returned of (display dialog \"PIN for {label}:\" default answer \"\" \
...
fn prompt(token_label: &str) -> Option<String> {
    ...
    let label = label_or_default(token_label).replace('\'', "''");
    let script = format!(
        r#"Add-Type -AssemblyName System.Windows.Forms,System.Drawing
$f = New-Object Windows.Forms.Form
$f.Text = 'OpenSigner'; $f.Width = 340; $f.Height = 170
$f.StartPosition = 'CenterScreen'; $f.TopMost = $true
$f.FormBorderStyle = 'FixedDialog'; $f.MinimizeBox = $false; $f.MaximizeBox = $false
$l = New-Object Windows.Forms.Label
$l.Text = 'PIN for {label}:'; $l.Left = 12; $l.Top = 16; $l.Width = 300
$t = New-Object Windows.Forms.TextBox
$t.UseSystemPasswordChar = $true; $t.Left = 12; $t.Top = 44; $t.Width = 300
$ok = New-Object Windows.Forms.Button
$ok.Text = 'OK'; $ok.Left = 144; $ok.Top = 84; $ok.DialogResult = 'OK'
$no = New-Object Windows.Forms.Button
$no.Text = 'Cancel'; $no.Left = 232; $no.Top = 84; $no.DialogResult = 'Cancel'
...
fn pinentry(text: &str) -> Option<String> {
        ...
        .ok()?;
    {
        ...
        let _ = writeln!(stdin, "SETDESC {text}");
