...
| D3 | **Update channel.** ~~Wire the host's `checkUpdate`.~~ Done: the extension forwards it, docsigner.js exposes `checkUpdate()`, and the default feed is the `latest.json` each release publishes. What's left is a surface that *shows* it — a banner on the consent page, and the desktop Settings panel. | The check was written, tested and documented, then blocked by the extension's own allowlist. Nothing failed; it just answered UNSUPPORTED. A js test now pins the allowlist against the commands CONTRACTS §2 defines. |
...
| D8 | **The `cryptography<49` pin in the desktop app.** Pinned because 49 dropped macOS x86_64 and universal2 wheels, and the `.app` is x86_64 so the sidecar can reach x86_64-only token drivers. | Watch pyca advisories. The day a fix lands only in 49+, drop the pin and build the `.app` arm64-native with the sidecar left at x86_64: they are separate processes, so the architectures need not match, and nothing in the Python process loads a PKCS#11 module. Cost of that switch is Intel Macs losing the desktop app, keeping the browser flow. Reasoning in full in `desktop/backend/requirements.txt`. |
